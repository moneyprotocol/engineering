...
/*
* PriceFeed for mainnet deployment, to be connected to Chainlink's live RBTC:USD aggregator reference 
...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
    ...
    // Core Moneyp contracts
    address borrowerOperationsAddress;
    address vaultManagerAddress;

    uint constant public RBTCUSD_TELLOR_REQ_ID = 1;
    ...
    */
    ...
    // The last good price seen from an oracle by Moneyp
    ...
    /*
    * fetchPrice():
    * Returns the latest price obtained from the Oracle. Called by Moneyp functions that require a current price.
    *
    * Also callable by anyone externally.
    *
    * Non-view function - it stores the last good price seen by Moneyp.
    *
    * Uses a main oracle (Chainlink) and a fallback oracle (Tellor) in case Chainlink fails. If both fail, 
    * it uses the last good price seen by Moneyp.
    *
    */
    ...
    */
    ...
    function _chainlinkPriceChangeAboveMax(ChainlinkResponse memory _currentResponse, ChainlinkResponse memory _prevResponse) internal pure returns (bool) {
        ...
        uint minPrice = MoneypMath._min(currentScaledPrice, prevScaledPrice);
        uint maxPrice = MoneypMath._max(currentScaledPrice, prevScaledPrice);
    ...
    function _bothOraclesSimilarPrice( ChainlinkResponse memory _chainlinkResponse, TellorResponse memory _tellorResponse) internal pure returns (bool) {
        ...
        uint minPrice = MoneypMath._min(scaledTellorPrice, scaledChainlinkPrice);
        uint maxPrice = MoneypMath._max(scaledTellorPrice, scaledChainlinkPrice);
    ...
    function _scaleChainlinkPriceByDigits(uint _price, uint _answerDigits) internal pure returns (uint) {
        /*
        * Convert the price returned by the Chainlink oracle to an 18-digit decimal for use by Moneyp.
        * At date of Moneyp launch, Chainlink uses an 8-digit price, but we also handle the possibility of
        * future changes.
        *
        */
        uint price;
        if (_answerDigits >= TARGET_DIGITS) {
            // Scale the returned price value down to Moneyp's target precision
            price = _price.div(10 ** (_answerDigits - TARGET_DIGITS));
        }
        else if (_answerDigits < TARGET_DIGITS) {
            // Scale the returned price value up to Moneyp's target precision
    ...
    function _getCurrentTellorResponse() internal view returns (TellorResponse memory tellorResponse) {
        try tellorCaller.getTellorCurrentValue(RBTCUSD_TELLOR_REQ_ID) returns
        (
            bool ifRetrieve,
            uint256 value,
