...
// Incremental changes (commit by commit) from the original to this version: https://github.com/moneyp/dev/pull/271
...
/*
 * On deployment a new Uniswap pool will be created for the pair BPD/RBTC and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of MP token,
...
contract RskSwapPool is LPTokenWrapper, Ownable, CheckContract, IRskSwapPool {
    ...
    function lastTimeRewardApplicable() public view override returns (uint256) {
        return MoneypMath._min(block.timestamp, periodFinish);
    }
    ...
    function claimReward() public override {
        ...
        mpToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == mpToken.balanceOf(address(this)));
