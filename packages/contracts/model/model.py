...
class ModelParams:
    def __init__(self):
        ...
        self.T = 5 # weighting for price change in vault issuance
        self.F = 5 # weighting for momentum change in vault issuance
...
# return the innate component of market demand for holding MP tokens.  Could be a function of:
...
def get_new_token_price(data, params, redeemed_amount, momentum):
    ...
    # price = (data.vault_issuance[-1] - data.token_demand[-1] - ((A + T) * data.token_price[-1]) + ((B + F) * momentum) - redeemed_amount) * factor 
    price = (data.vault_issuance[-1] - data.innate_token_demand - (A * data.token_price[-1] )  -T + ((B + F) * momentum) - redeemed_amount) * factor 
...
def get_new_vault_issuance(data, params, token_price, momentum ):
    vault_issuance = data.vault_issuance[-1] + params.T*(token_price - 1) + params.F*(momentum)
    if vault_issuance < 0:
        return 0
    else: 
