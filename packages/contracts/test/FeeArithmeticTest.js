...
contract('Fee arithmetic tests', async accounts => {
  ...
  it("minutesPassedSinceLastFeeOp(): returns minutes passed for no time increase", async () => {
    await vaultManagerTester.setLastFeeOpTimeToNow()
    const minutesPassed = await vaultManagerTester.minutesPassedSinceLastFeeOp()
  ...
  it("decayBaseRateFromBorrowing(): returns the initial base rate for no time increase", async () => {
    await vaultManagerTester.setBaseRate(dec(5, 17))
    await vaultManagerTester.setLastFeeOpTimeToNow()

    const baseRateBefore = await vaultManagerTester.baseRate()
    assert.equal(baseRateBefore, dec(5, 17))

    await vaultManagerTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter = await vaultManagerTester.baseRate()
  ...
  it("decayBaseRateFromBorrowing(): returns the initial base rate for less than one minute passed ", async () => {
    ...
    // 1 second
    const baseRateBefore_1 = await vaultManagerTester.baseRate()
    assert.equal(baseRateBefore_1, dec(5, 17))

    await th.fastForwardTime(1, web3.currentProvider)

    await vaultManagerTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter_1 = await vaultManagerTester.baseRate()
    ...
    const baseRateBefore_2 = await vaultManagerTester.baseRate()
    await th.fastForwardTime(17, web3.currentProvider)

    await vaultManagerTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter_2 = await vaultManagerTester.baseRate()
    ...
    const baseRateBefore_3 = await vaultManagerTester.baseRate()
    await th.fastForwardTime(29, web3.currentProvider)

    await vaultManagerTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter_3 = await vaultManagerTester.baseRate()
    ...
    const baseRateBefore_4 = await vaultManagerTester.baseRate()
    await th.fastForwardTime(50, web3.currentProvider)

    await vaultManagerTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter_4 = await vaultManagerTester.baseRate()
  ...
  it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.01", async () => {
    ...
    for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
      // Set base rate to 0.01 in VaultManager
      await vaultManagerTester.setBaseRate(dec(1, 16))
      const contractBaseRate = await vaultManagerTester.baseRate()
      ...
      await vaultManagerTester.unprotectedDecayBaseRateFromBorrowing()
      const decayedBaseRate = await vaultManagerTester.baseRate()
  ...
  it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.1", async () => {
    // baseRate = 0.1
    for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
      // Set base rate to 0.1 in VaultManager
      await vaultManagerTester.setBaseRate(dec(1, 17))
      const contractBaseRate = await vaultManagerTester.baseRate()
  ...
  it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.34539284", async () => {
    ...
    for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
      // Set base rate to 0.1 in VaultManager
      await vaultManagerTester.setBaseRate('345392840000000000')
      const contractBaseRate = await vaultManagerTester.baseRate()
      await vaultManagerTester.setBaseRate('345392840000000000')
  ...
  it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.9976", async () => {
    // baseRate = 0.9976
    for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
      // Set base rate to 0.9976 in VaultManager
      await vaultManagerTester.setBaseRate('997600000000000000')
      await vaultManagerTester.setBaseRate('997600000000000000')
