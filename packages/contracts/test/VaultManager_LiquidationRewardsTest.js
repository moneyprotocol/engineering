...
contract('VaultManager - Redistribution reward calculations', async accounts => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    const alice_Coll = ((await vaultManager.Vaults(alice))[1]
      .add(await vaultManager.getPendingETHReward(alice)))
      .toString()
    const carol_Coll = ((await vaultManager.Vaults(carol))[1]
      .add(await vaultManager.getPendingETHReward(carol)))
      .toString()
  ...
  it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
    ...
    const bob_Coll = ((await vaultManager.Vaults(bob))[1]
      .add(await vaultManager.getPendingETHReward(bob)))
      .toString()
    const dennis_Coll = ((await vaultManager.Vaults(dennis))[1]
      .add(await vaultManager.getPendingETHReward(dennis)))
      .toString()
    const erin_Coll = ((await vaultManager.Vaults(erin))[1]
      .add(await vaultManager.getPendingETHReward(erin)))
      .toString()
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving vault has RBTC from all previously liquidated vaults", async () => {
    ...
    const freddy_ETHReward = (await vaultManager.getPendingETHReward(freddy)).toString()
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const B_ETHGain_1 = await vaultManager.getPendingETHReward(B)
    const C_ETHGain_1 = await vaultManager.getPendingETHReward(C)
    const D_ETHGain_1 = await vaultManager.getPendingETHReward(D)
    const E_ETHGain_1 = await vaultManager.getPendingETHReward(E)
    ...
    assert.isAtMost(getDifference(B_expectedPendingRBTC_1, B_ETHGain_1), 1e8)
    assert.isAtMost(getDifference(C_expectedPendingRBTC_1, C_ETHGain_1), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingRBTC_1, D_ETHGain_1), 1e8)
    assert.isAtMost(getDifference(E_expectedPendingRBTC_1, E_ETHGain_1), 1e8)
    ...
    const B_ETHGain_2 = await vaultManager.getPendingETHReward(B)
    const D_ETHGain_2 = await vaultManager.getPendingETHReward(D)
    const E_ETHGain_2 = await vaultManager.getPendingETHReward(E)
    ...
    assert.isAtMost(getDifference(B_expectedPendingRBTC_2, B_ETHGain_2), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingRBTC_2, D_ETHGain_2), 1e8)
    assert.isAtMost(getDifference(E_expectedPendingRBTC_2, E_ETHGain_2), 1e8)
    ...
    const B_ETHGain_3 = await vaultManager.getPendingETHReward(B)
    const D_ETHGain_3 = await vaultManager.getPendingETHReward(D)
    ...
    assert.isAtMost(getDifference(B_expectedPendingRBTC_3, B_ETHGain_3), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingRBTC_3, D_ETHGain_3), 1e8)
  })
  ...
  it("redistribution: Vault with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    const alice_ETHReward_1 = await vaultManager.getPendingETHReward(alice)
    const bob_ETHReward_1 = await vaultManager.getPendingETHReward(bob)
    const carol_ETHReward_1 = await vaultManager.getPendingETHReward(carol)
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    ...
    assert.isAtMost(th.getDifference(await vaultManager.getPendingETHReward(bob), B_pendingRewardsAfterL1), 1000000)
    assert.isAtMost(th.getDifference(await vaultManager.getPendingETHReward(carol), C_pendingRewardsAfterL1), 1000000)
    ...
    assert.isAtMost(th.getDifference(await vaultManager.getPendingETHReward(carol), C_pendingRewardsAfterL2), 1000000)
    assert.isAtMost(th.getDifference(await vaultManager.getPendingETHReward(dennis), D_pendingRewardsAfterL2), 1000000)
    ...
    const carol_pendingETHReward = (await vaultManager.getPendingETHReward(carol)).toString()

    const dennis_rawColl = (await vaultManager.Vaults(dennis))[1].toString()
    const dennis_pendingETHReward = (await vaultManager.getPendingETHReward(dennis)).toString()

    const erin_rawColl = (await vaultManager.Vaults(erin))[1].toString()
    const erin_pendingETHReward = (await vaultManager.getPendingETHReward(erin)).toString()
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    */
    ...
    const D_pendingRewardsAfterL2 = D_coll.mul(th.applyLiquidationFee(B_collAfterL1)).div(C_collAfterL1.add(D_coll))
    const C_collAfterL2 = C_collAfterL1.add(C_pendingRewardsAfterL2)
    assert.isAtMost(th.getDifference(await vaultManager.getPendingETHReward(carol), C_pendingRewardsAfterL2), 10000000)
    assert.isAtMost(th.getDifference(await vaultManager.getPendingETHReward(dennis), D_pendingRewardsAfterL2), 10000000)
