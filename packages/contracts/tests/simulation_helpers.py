...
# number of liquidations for each call to `liquidateVaults`
NUM_LIQUIDATIONS = 10
...
Bitcoin is the collateral for BPD. The ether price $P_t^e$ follows 
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop MP gain.
...
Each vault is defined by five numbers
> (collateral in ether, debt in BPD, collateral ratio target, rational inattention, collateral ratio)
...
The amount of new vaults opened in period t is denoted by $N_t^o$, which follows 
...
where the shock $\zeta_t^o \sim N(0,sd\_openvaults)$. 

$R_t^o$ represents the break-even natural rate of opening vaults and $f_t^i$ represents the issuance fee.

$P_{t}^{l}$ is the price of BPD.

$N_t^o$ is rounded to an integer.

---

The amount of BPD tokens generated by a new vault is
...
---
Each vault is associated with a rational inattention parameter $\tau(i)$.

The collateral ratio of the existing vaults vary with the ether price $P_t^e$
> $$CR_t(i) = \frac{P_t^e Q_t^e(i)}{Q_t^d(i)}.$$

If the collateral ratio falls in the range 
> $CR_t(i) \in [CR^*(i)-\tau(i), CR^*(i)+2\tau(i)]$,

no action taken. Otherwise, the vault owner readjusts the collateral ratio so that
>  $CR_t(i)=CR^*(i)$.
...
The amount of vaults closed in period t is denoted as $N_t^c$, which follows
> $$N_t^c = \begin{cases} 
...
where the shock $\zeta_t^c \sim N(0, sd\_closevaults)$. 
...
the system checks the collateral ratio of the exisitng vaults in the
vault pool. 

If the collateral ratio falls below 110%, i.e.
> $$CR_t(i) = \frac{P_t^e Q_t^e(i)}{Q_t^d(i)}<110\%,$$

this vault is liquidated. Namely, it is eliminated from the vault pool.

Denote the amount of liquidated vaults by $N_t^l$. The sum of the debt amounts to
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of MP token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of BPD deposited in the Stability Pool (see below)
...
def pending_liquidations(contracts, price_ether_current):
    ...
    last_ICR = contracts.vaultManager.getCurrentICR(last_vault, Wei(price_ether_current * 1e18))

    if last_vault == ZERO_ADDRESS:
        return False
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalBPDDeposits()
    vault = last_vault
    for i in range(NUM_LIQUIDATIONS):
        debt = contracts.vaultManager.getEntireDebtAndColl(vault)[0]
        if stability_pool_balance >= debt:
            return True
        vault = contracts.sortedVaults.getPrev(vault)
...
def liquidate_vaults(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_BPD, data, index):
    ...
    while pending_liquidations(contracts, price_ether_current):
        ...
        #print(tx.events['VaultLiquidated'])
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_BPD
    ...
    return_stability = calculate_stability_return(contracts, price_BPD, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_BPD, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalBPDDeposits() / 1e18
    if index == 0:
    ...
    if stability_pool_previous == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_BPD * stability_pool_previous)
    else:
...
def close_vaults(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_BPD, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    n_vaults = contracts.sortedVaults.getSize()
    ...
    if index <= 240:
        ...
        number_closevaults = max(0, n_steady * (1+shock_closevaults)) + beta*(1-price_BPD)*n_vaults

    number_closevaults = min(int(round(number_closevaults)), len(active_accounts) - 1)
    random.seed(293+100*index)
    drops = list(random.sample(range(len(active_accounts)), number_closevaults))
    for i in range(0, len(drops)):
        account_index = active_accounts[drops[i]]['index']
        account = accounts[account_index]
        pending = get_bpd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, get_total_debt(contracts, account))
        if pending == 0:
            # TODO: try to predict it!
            try: # to skip “BorrowerOps: An operation that would result in TCR < CCR is not permitted” errors
                contracts.borrowerOperations.closeVault({ 'from': account })
    ...
    return [number_closevaults]

"""Adjust Vaults"""

def get_total_coll(contracts, account):
    amounts = contracts.vaultManager.getEntireDebtAndColl(account)
    return amounts['coll']
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.bpdToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.bpdToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_bpd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    bpdBalance = contracts.bpdToken.balanceOf(account)
    if debt > bpdBalance:
        pending = debt - bpdBalance
        # first try with whale
        pending = transfer_from_to(contracts, accounts[0], account, pending)
        # first try with active accounts, which are more likely to hold BPD
        ...
        if pending > 0:
            print(f"\n ***Error: not enough BPD to repay! {debt / 1e18} BPD for {account}")
...
def get_hints(contracts, coll, debt):
    ...
    return contracts.sortedVaults.findInsertPosition(NICR, approxHint[0], approxHint[0])
...
def get_hints_from_ICR(accounts, contracts, active_accounts, ICR, NICR):
    ...
    if l == 0:
        ...
        hints = contracts.sortedVaults.findInsertPosition(
            NICR,
...
def adjust_vaults(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    random.seed(57984-3*index)
    ratio = random.uniform(0,1)
    coll_added_float = 0

    for i, working_vault in enumerate(active_accounts):
        account = accounts[working_vault['index']]
        currentICR = contracts.vaultManager.getCurrentICR(account, floatToWei(price_ether_current)) / 1e18
        coll = get_total_coll(contracts, account) / 1e18
        debt = get_total_debt(contracts, account) / 1e18

        random.seed(187*index + 3*i)
        p = random.uniform(0,1)
        check = (currentICR - working_vault['CR_initial']) / (working_vault['CR_initial'] * working_vault['Rational_inattention'])
        ...
        if p >= ratio:
            debt_new = price_ether_current * coll / working_vault['CR_initial']
            ...
            if check < -1:
                ...
                pending = get_bpd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayBPD(repay_amount, hints[0], hints[1], { 'from': account })
            elif check > 2 and not is_recovery_mode(contracts, price_ether_current):
                # withdraw BPD
                withdraw_amount = floatToWei(debt_new - debt)
                try: # to skip “BorrowerOps: An operation that would result in TCR < CCR is not permitted” errors
                    contracts.borrowerOperations.withdrawBPD(MAX_FEE, withdraw_amount, hints[0], hints[1], { 'from': account })
                except:
                    print("\n ***Error withdrawing BPD!")
        #Another part of the vaults are adjusted by adjusting collaterals
        elif p < ratio:
            coll_new = working_vault['CR_initial'] * debt / price_ether_current
            ...
            if check < -1:
                ...
                try: # to skip “BorrowerOps: An operation that would result in TCR < CCR is not permitted” errors
                    ...
                    print("\n ***Error withdrawing RBTC!")

    return coll_added_float

"""Open Vaults"""

def open_vault(accounts, contracts, active_accounts, inactive_accounts, supply_vault, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    hints = get_hints_from_amounts(accounts, contracts, active_accounts, quantity_ether, supply_vault, price_ether_current)
    try: # to skip “BorrowerOps: An operation that would result in TCR < CCR is not permitted” errors
        contracts.borrowerOperations.openVault(MAX_FEE, floatToWei(supply_vault), hints[0], hints[1],
        ...
        print(f"$/RBTC {price_ether_current}")
        print(f"H ICR {contracts.vaultManager.getCurrentICR(hints[0], Wei(price_ether_current * 1e18))}")
        print(f"H ICR {contracts.vaultManager.getCurrentICR(hints[1], Wei(price_ether_current * 1e18))}")

def open_vaults(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_BPD, index):
    random.seed(2019*index)
    shock_openvaults = random.normalvariate(0,sd_openvaults)
    n_vaults = len(active_accounts)
    rate_issuance = contracts.vaultManager.getBorrowingRateWithDecay() / 1e18
    coll_added = 0

    if index <= 0:
        number_openvaults = initial_open
    elif price_BPD <= 1 + rate_issuance:
        number_openvaults = max(0, n_steady * (1+shock_openvaults))
    else:
        number_openvaults = max(0, n_steady * (1+shock_openvaults)) + \
                        alpha * (price_BPD - rate_issuance - 1) * n_vaults

    number_openvaults = min(int(round(float(number_openvaults))), len(inactive_accounts))
    ...
    for i in range(0, number_openvaults):
        ...
        if supply_vault < MIN_NET_DEBT:
            supply_vault = MIN_NET_DEBT
            quantity_ether = CR_ratio * supply_vault / price_ether_current

        open_vault(accounts, contracts, active_accounts, inactive_accounts, supply_vault, quantity_ether, CR_ratio, rational_inattention, price_ether_current)
...
def stability_update(accounts, contracts, return_stability, index):
    supply = contracts.bpdToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalBPDDeposits() / 1e18
    ...
    if index <= month:
        ...
        if stability_pool > stability_pool_previous:
            ...
            whale_balance = contracts.bpdToken.balanceOf(accounts[0]) / 1e18
            ...
            current_deposit = contracts.stabilityPool.getCompoundedBPDDeposit(accounts[0])
...
---
With the supply and demand of BPD tokens defined above, the price of BPD at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of BPD.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_BPD, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_BPD_current= price_BPD * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_BPD_current

""" **Stabilizers**

There are two stabilizers to attenuate BPD price deviation from its target range.
...
If $P_t^l > 1.1+f_t^i$, open a new vault with $CR^*=110\%$ and $\tau^*=10\%$. Its debt amounts to
> $$Q_t^d(c) = \frac{P_t^e Q_t^e(c)}{110\%}.$$

The amount of $Q_t^d(c)$ is expected to bring the BPD price back to $1.1+f_t^i$. This means that
> $$S_t' = D_t^s + (\frac{1.1+f_t^i}{P_{t-1}^l})^\delta D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)$$

The debt of th new vault is the difference between the original supply and the supply needed to bring price to $1.1+f_t^i$, which is
> $$Q_t^d(c) = S_t' - S_t$$
...
$P_t^l = 1.1+f_t^i$ ==> denote this amount by $Q_t^d(c)$ ==> open a vault
with $CR^*=110\%$ and debt = $Q_t^d(c)$

---
Stabilizer 2: floor arbitrageurs

If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of BPD in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$

where
> $$\chi_t = ...$$

The redemption eliminates vaults with the lowest collateral ratio.

Note that unlike stabilizer 1, stabilizer 2 has impact of BPD price in
...
program stabilizer 2 like what you did for stabilizer 1. The redemption kills some vaults and thus affect $P_{t+1}^l$ in the next period as the number of vaults shrinks.

**Programming logic**

Denote the amount of vaults fully redeemed by $N_t^r$. Therefore,
> $$D_t^r = \sum_i^{N_t^r} Q_t^d(i) + \Delta$$

where $\Delta \geq 0$ represents the residual.

Note that the redemption starts from the riskest vaults, i.e. those with
the lowest collateral ratios.

If any residual $\Delta > 0$ left, then the changes to the vault $j$ with the lowest collateral ratio are
...
def price_stabilizer(accounts, contracts, active_accounts, price_BPD, index):

    stability_pool = contracts.stabilityPool.getTotalBPDDeposits() / 1e18
    redemption_pool = 0
    ...
    #Calculating Price
    price_BPD_current = calculate_price(price_BPD, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.vaultManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.vaultManager.getRedemptionRateWithDecay() / 1e18
    ...
    if price_BPD_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_BPD)**delta
        supply_vault = min(supply_wanted - supply, MIN_NET_DEBT)

        CR_ratio = 1.1
        rational_inattention = 0.1
        quantity_ether = supply_vault * CR_ratio / price_ether_current
        open_vault(accounts, contracts, active_accounts, inactive_accounts, supply_vault, quantity_ether, CR_ratio, rational_inattention)
    ...
    if price_BPD_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_BPD)**delta
        ...
        if redemption_pool > whale_balance:
            ...
            tx = contracts.vaultManager.redeemCollateral(floatToWei(redemption_pool), { 'from': accounts[0] })
            remove_accounts_from_events(
                accounts,
                active_accounts,
                inactive_accounts,
                filter(lambda e: e['coll'] == 0, tx.events['VaultUpdated']),
                '_borrower'
            )


    return [price_BPD_current, redemption_pool]
