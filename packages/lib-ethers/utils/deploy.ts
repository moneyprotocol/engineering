...
const deployContracts = async (
  ...
  const addresses = {
    activePool: await deployContract(
      deployer,
      getContractFactory,
      "ActivePool",
      { ...overrides }
    ...
    mpStaking: await deployContract(deployer, getContractFactory, "MPStaking", {
      ...overrides,
    }),
    ...
    sortedVaults: await deployContract(
      deployer,
      getContractFactory,
      "SortedVaults",
      {
...
const connectContracts = async (
  ...
  const txCount = await deployer.provider.getTransactionCount(
    deployer.getAddress()
  );

  const connections: ((nonce: number) => Promise<ContractTransaction>)[] = [
    (nonce) =>
      sortedVaults.setParams(
        1e6,
...
export const deployAndSetupContracts = async (
  ...
  const mpTokenDeploymentTime =
    await contracts.mpToken.getDeploymentStartTime();
  const bootstrapPeriod = await contracts.vaultManager.BOOTSTRAP_PERIOD();
  const totalStabilityPoolMPReward =
    await contracts.communityIssuance.MPSupplyCap();
