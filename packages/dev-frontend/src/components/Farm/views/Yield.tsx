...
export const Yield: React.FC = () => {
  ...
  const { remainingLiquidityMiningMPReward, totalStakedUniTokens } = useMoneypSelector(selector);
  ...
  const hasZeroValue = remainingLiquidityMiningMPReward.isZero || totalStakedUniTokens.isZero;
  ...
  useEffect(() => {
    (async () => {
      try {
        const { mpPriceUSD, uniLpPriceUSD } = await fetchPrices(mpTokenAddress, uniTokenAddress);
  ...
  }, [mpTokenAddress, uniTokenAddress]);
